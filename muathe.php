  ...
  $loaithe = tmq_boc($_POST['type']);
    $menhgia = tmq_boc($_POST['amount']);
        ...
        if(empty($loaithe) || empty($menhgia) || empty($soluong)){
            echo "Không được để trống trường hợp nào";
        }elseif(empty($uid)){
            echo "Vui lòng đăng nhập để thực hiện giao dịch này.";
        }elseif($TMQ['cash'] < $total){
            echo "Tài khoản của bạn không đủ để thực hiện giao dịch này.";
        }elseif($get['ten'] == null){
            echo "Loại thẻ không tồn tại";
        }elseif($check_kho->fetch()['id'] <= $soluong){
            echo "Kho đã hết thẻ, vui lòng quay lại sau.";    
        }else{
...
$db->exec("UPDATE `TMQ_loaithe` SET `soluong` = `soluong` - '1' WHERE `menhgia` = '$menhgia' AND `loaithe` = '$loaithe'");
 ...
 $db->exec("INSERT INTO `TMQ_biendoi` SET
   ...
   `saugd` = '$saugd',
    `time` = '". time() ."'
    ");
    echo "Thành công, check lịch sử mua thẻ để lấy mã thẻ";
        }
